...
describe('packaged CLI docs', () => {
  it('retrieves packaged topics without the monorepo docs tree', () => {
    ...
    expect(existsSync(path.join(cliRoot, 'dist/docs/topics/topics.json'))).toBe(
      true,
    );
    expect(
    ...
    const packDir = mkdtempSync(path.join(os.tmpdir(), 'delivery-docs-pack-'));
